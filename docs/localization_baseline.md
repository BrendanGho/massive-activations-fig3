# Localization baseline — Figure 3 protocol
...
This is the inherited baseline protocol. See [the project overview](../README.md)
for the channel-stability and norm-decomposition studies. Paths below are relative
...
**Historical comparison targets** (only checked with `--reference-check`): top-k dominates every layer,
bottom-k flat ≈ 0.2, random-k between, FLUX.2-klein top-k peak ≈ 0.5 near layer 10.
These are inherited approximate expectations, not correctness criteria or required
research outcomes. A different curve can reflect the model, data, or settings. The
default evaluation checks numerical validity without requiring a particular ordering.


The study launcher uses neutral artifact names:

```bash
python -m src.experiments localization --config configs/default.yaml
```

This writes `localization_results.csv` and `localization_curve.png`. The historical
`python -m src.stage4_evaluate_figure3d` command retains `figure3d_*` filenames.
Both commands accept `--artifact-prefix localization|figure3d` and the optional
`--reference-check`. Existing `FIG3_*` configuration overrides remain supported.
